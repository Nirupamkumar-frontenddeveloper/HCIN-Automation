...
// records across 8 logins".
const DESTRUCTIVE_PATTERN = /delete|reject|approve|cancel|deactivate|disable|remove|terminate|block/i;
...
// "Cases"), switches through every tab a list page has (e.g. Programs' "All List"/
// "Awaiting Approval"), and on each one exercises the first row's Action control - every
// item inside its dropdown, not just the first - plus every download control on the page,
// registering Pass/Fail based on whether the resulting API call(s) returned 200.
class LosCrawlerPage extends BasePage {
    ...
    // Some list pages have their own secondary tabs (e.g. Programs' "All List"/"Awaiting
    // Approval") that show entirely different rows/data. Scoped to role=tab, which is the
    // pattern this app already uses consistently for that (confirmed in ProgramsPage.js).
    // Some pages (e.g. LMS's "System Configuration") nest a SECOND tablist inside each
    // outer tab's panel (numbered outer tabs "1/2/3/4", each containing its own named
    // inner tabs like "Currency"/"Payments"). The inner tablist sits inside the outer
    // tablist's own DOM subtree (not a later sibling), so scoping to "the first tablist"
    // still picks up the nested tabs too. What actually distinguishes an outer tab is
    // that its own tablist has no further tablist wrapping it - counting tablist
    // ancestors tells outer (depth 1) apart from nested (depth 2+). Without this,
    // switchToTab() tries to click an inner tab while a different outer tab than the one
    // it belongs to is selected, where it isn't attached/visible, and hangs until timeout.
    ...
    // Some stepper-style tabs (numbered "1/2/3/4" steps) carry a visually-hidden
    // duplicate text node, so raw textContent() returns something like "1 1" instead of
    // "1" - that doesn't match what getByRole's own accessible-name computation resolves
    // it to, so switchToTab() would never find it. ariaSnapshot() reports the same
    // computed accessible name getByRole matches against, so reading the name from there
    // instead keeps discovery and matching consistent.
    ...
    // (Approve/Reject/Delete/...) is deliberately skipped rather than auto-clicked, and
    // recorded as an intentional Pass so it's visible in the report without being fired.
    // If the row has no dropdown (just a plain link), that link is clicked instead, same
    // as before. Pass/Fail is based on whether the resulting API call(s) returned 200.
    // `tabName`, when the page has tabs, is re-selected after every recovery navigation so
    // a menu that needs repeated switching between e.g. "All List" and "Awaiting Approval"
    // never silently drifts onto the wrong tab's data mid-way through.
    ...
    // controls show up both inside row-level Action menus (e.g. a document's download
    // icon) and as standalone page-level buttons (e.g. a report's "Download Report"
    // button) - and some list pages have one per row, so this returns all of them.
